...
if (fs.existsSync(themeContextPath)) {
  ...
  // Find where the rose-gold entry ends
  const roseGoldIndex = content.indexOf("'rose-gold': {");
  if (roseGoldIndex !== -1) {
    const nextCloseBraceIndex = content.indexOf('},', roseGoldIndex);
    if (nextCloseBraceIndex !== -1) {
      const targetStr = content.substring(roseGoldIndex, nextCloseBraceIndex + 2);
      ...
      console.log('Successfully patched ThemeContext.tsx');
    } else {
      console.error('Could not find closing brace for rose-gold');
    }
  } else {
    console.error('Could not find rose-gold key');
  }
} else {
  console.error('File not found:', themeContextPath);
}
